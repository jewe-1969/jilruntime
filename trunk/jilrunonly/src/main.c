...
// THROW
//------------------------------------------------------------------------------
// macro for error handling - to avoid obscuring the code by repetitive error
// handling blocks.
...
const char* kForwardDeclareMain =
	"function string main(const string[] args);" TAG("This is the main entry point function for any script executed by the command line application. Implement this function in your script. Any command line arguments will be passed as a string array in 'args'.");
...
int main(int nArgs, char* ppArgList[])
{
	...
	// install a log message handler (optional, recommended)
	...
	// for simplicity, we only handle 2 of the 4 possible types of exceptions
	...
	// Enable fixed memory management (optional, recommended)
...
// To clear the exception state, the handler can call JILClearExceptionState().
...
// the JILRuntime library. You may override this by defining JIL_RUNTIME_CHECKS
// to 1 when building the library. However, this will have a huge impact on the
// performance of the VM.
// Exceptions that are always invoked are the null-reference exception and the
